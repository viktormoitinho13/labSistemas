...
(
	...
	ALTER TABLE `PESSOA_FISICA` 
		ADD CONSTRAINT `PESSOA_FISICA_fk0` 
			FOREIGN KEY (`LOJA_FUNC`) 
				REFERENCES `LOJAS`(`ID_LOJA`);

	ALTER TABLE `FUNCIONARIO` 
		ADD CONSTRAINT `FUNCIONARIO_fk0` 
			FOREIGN KEY (`ID_FUNC`) 
				REFERENCES `PESSOA_FISICA`(`ID_PI`);

	ALTER TABLE `CARGO` 
		ADD CONSTRAINT `CARGO_fk0` 
			FOREIGN KEY (`ID_CARGO`) 
				REFERENCES `PESSOA_FISICA`(`ID_PI`);

	ALTER TABLE `LOGIN_LOJA` 
		ADD CONSTRAINT `LOGIN_LOJA_fk0` 
			FOREIGN KEY (`ID_LOGIN`) 
				REFERENCES `LOJAS`(`ID_LOJA`);

	ALTER TABLE `PRODUTO` 
		ADD CONSTRAINT `PRODUTO_fk0` 
			FOREIGN KEY (`ID_PRODUTO`) 
				REFERENCES `ESTOQUE`(`ID_ESTOQUE`);

	ALTER TABLE `ESTOQUE` 
		ADD CONSTRAINT `ESTOQUE_fk0` 
